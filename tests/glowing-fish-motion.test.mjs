...
test('GlowingFish update keeps the spatial hash and consumes the new motion axes', () => {
  ...
  assert.match(source, /xDistSq \+ yDistSq \+ zDistSq > radiusSq/)
  ...
  assert.match(source, /detailedSchoolRadius/)
  assert.match(source, /if \(canSeparate\) \{[\s\S]*?const dist = Math\.sqrt\(dSq\)/)
